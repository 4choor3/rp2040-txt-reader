# RP2040 开发板实践 · 制作指南（中文）

本指南按**真实制作时序**编排：**选型 → 烧录固件（先让板子能跑）→ 接线 → 组装 → 通电检查 → 放文件使用 → 故障排查**。烧录只需要主控板 + USB 线，无需等焊接完成，因此放在最前面——先验证板子是好的，再动手焊，能省去大量排障时间。

## 一、硬件选型说明（为什么是这套组合）

本项目追求两条硬性目标：**极低成本（整机 < 30 元）**、**免编译/免重烧即可放文件**。

### 1. 主控方案对比

| 主控方案 | 架构 | 本地存储 | PC 放文件方式 | 成本 | 备注 |
|----------|------|----------|-------------|------|------|
...
**结论**：RP2040 自带的原生 USB 控制器 + CircuitPython 的 USB MSC 协议，把"放文件"变成插线拖文件，体验最佳、开发最简单。

### 2. 屏幕方案对比

| 屏幕型号 | 尺寸/分辨率 | 价格 | 适合显示文本的原因 | 缺点 |
|----------|-------------|------|----------------|------|
