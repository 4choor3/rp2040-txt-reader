...
> 拿 **YD-RP2040**（16MB）开发板接一块 **1.8 寸 ST7735 TFT** 屏做的硬件实践，跑 **CircuitPython 9.x**。第一次碰这类开发板，从选型、接线到焊接装配都是自己摸的。
...
一块板子 + 一块屏幕，叠成火柴盒大小的「三明治」。整机物料成本 **低于 ¥30**，全部使用市面通用模块。
...
rp2040-txt-reader/
├── README.md                  # 本文件
├── code.py                    # 主程序：文本显示与翻页（CircuitPython 9.x）
├── LICENSE                    # MIT License
├── assets/                    # 实机演示图与视频
...
6. **尺寸与供电。** 53.3×22.9 mm 主控正好藏在约 56×34 mm 屏幕背后；**503035** 锂电池 + **TP4056** 充放电板供电。
